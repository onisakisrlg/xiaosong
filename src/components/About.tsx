...
export const About: React.FC = () => {
  ...
  const companyFacts = [
    ...
    {
      icon: <ClipboardCheck className="w-5 h-5 text-slate-300" />,
        ...
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-8">
          ...
          <div className="lg:col-span-6 space-y-6">
            <div className="bg-slate-950/70 border border-slate-800/80 p-6 sm:p-8 rounded-2xl shadow-xl shadow-black/30 relative">
              <div className="absolute top-6 right-6 flex items-center space-x-1.5 bg-slate-900 border border-slate-700 px-3 py-1 rounded-full">
                <div className="w-2 h-2 rounded-full bg-slate-400" />
                <span className="text-[10px] uppercase font-mono tracking-wider font-semibold text-slate-300">正規法人登記</span>
              </div>
              
              <h3 className="text-xl font-bold text-slate-100 mb-6 flex items-center space-x-2">
                <span className="w-1 h-5 bg-white rounded" />
                <span>会社基本情報</span>
              </h3>
                  ...
                  <div key={index} className="flex items-start pb-4 border-b border-slate-800/40 last:border-b-0 last:pb-0">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm sm:text-base text-slate-200 font-medium leading-relaxed">
                        {fact.value}
                        {fact.badge && (
                          <span className="ml-2.5 bg-slate-850 border border-slate-700 text-slate-300 font-mono text-[10px] px-2 py-0.5 rounded font-semibold">
                            {fact.badge}
                          </span>
                        )}
                        {fact.searchUrl && (
                          <a
                            ...
                            className="inline-flex items-center space-x-1 ml-2 text-slate-300 hover:text-white text-xs transition-colors underline"
                          >
                            <span>地图查看</span>
            ...
            <div className="bg-slate-950/40 border border-slate-800 p-5 rounded-xl flex items-center space-x-4">
              <div className="w-12 h-12 rounded-lg bg-slate-900 flex items-center justify-center border border-slate-800 shrink-0">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">※ 近江北线、江北出口。日本全国大规格卡车载重规格轮胎均可调拨配载。</p>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-6 flex flex-col justify-between">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {valueProps.map((prop, idx) => (
                <div key={idx} className="bg-slate-950/40 border border-slate-800 p-5 rounded-xl hover:border-slate-750 transition-all duration-300">
                  <div className="w-10 h-10 rounded-lg bg-slate-900 border border-slate-800 flex items-center justify-center font-bold text-white font-mono text-sm mb-4">
                    0{idx + 1}
                  </div>
            ...
            <div className="mt-8 p-5 rounded-xl border border-slate-800 bg-slate-950/60 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-white/5 -rotate-45 pointer-events-none rounded-full blur-xl" />
              <div className="flex items-start space-x-3">
                <Award className="w-6 h-6 text-white shrink-0 mt-0.5" />
                <div>
                  <p className="text-xs font-semibold text-slate-300 font-mono tracking-widest uppercase mb-1">官方正规商号</p>
                  <p className="text-sm text-slate-330 leading-relaxed">
